return {
  ...
  config = function()
    ...
    vim.keymap.set({ "n", "x" }, "<leader>m<C-j>", add_cursor_down, { desc = "Add cursor down" })
    vim.keymap.set({ "n", "x" }, "<leader>m<C-k>", add_cursor_up, { desc = "Add cursor up" })
    vim.keymap.set({ "n", "x" }, "<leader>ma", match_all, { desc = "Add cursor to all matches" })
